...
```{r}
ipsc_obj$time_category <- case_when(ipsc_obj$batch == "batch1" ~ "early", 
                                    ipsc_obj$batch == "batch2" ~ "intermediate",
                                    ipsc_obj$batch == "batch3" ~ "late")
table(ipsc_obj$time_category)
```

```{r fig.height=10}
time_cat_umap <- custom_dimplot(ipsc_obj@meta.data, 
...
# pdf("figures/day0_day50/CTMR/iPSC_day0-day50_time_category_UMAP.pdf", height = 12, width = 10)
...
plots <- purrr::map(names(genes), function(geneset){
