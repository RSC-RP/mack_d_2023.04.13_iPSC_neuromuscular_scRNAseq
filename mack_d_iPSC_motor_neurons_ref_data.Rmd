...
```{r}
table(subset_common_types$top_level_annotation, subset_common_types$dataset)
```


```{r fig.height=10, fig.width=10}
types_across_ds <- unique(grep("OPC|Astr|Oligo|Endo", top_level_table_merged$top_level_annotation, value=TRUE, ignore.case = TRUE))
# subset_common_types <- subset(merged_split_obj, top_level_annotation %in% types_across_ds)
...
```{r}
length(unique(merged_split_obj$subtype_annotation))
```
