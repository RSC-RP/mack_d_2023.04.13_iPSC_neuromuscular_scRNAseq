...
embryonic_sc$donor <- embryonic_sc$orig_replicate
...
merged_obj@meta.data %>% 
  ...
  ungroup() %>% 
  arrange(desc(n)) %>% 
  filter(grepl("Moto|MN|pMN", subtype_annotation))
```
...
# merged_split_obj
merged_split_obj$motorneuron <- ifelse(grepl("Moto|MN|pMN",merged_split_obj$subtype_annotation), "MN", "Other")
