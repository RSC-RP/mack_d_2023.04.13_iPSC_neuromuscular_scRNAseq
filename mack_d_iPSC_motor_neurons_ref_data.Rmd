...
rayon_degs <- openxlsx::read.xlsx("resources/cell_refs/Yadav_2023/1-s2.0-S0896627323000314-mmc5.xlsx", sheet = 1, check.names = FALSE, sep.names = " ") %>% 
  pivot_longer(cols = everything(),
               ...
               values_to = "degs") %>% 
  mutate(top_level_annotation = gsub("-[0-9]{1,2}$","", subtype_annotation))

# esc_celltypes <- unique(rayon_degs$subtype_annotation)
# rayon_deg_gene_list <-  purrr::map(, function(x){
#   rayon_degs %>% 
#     filter(subtype_annotation == x) %>% 
#     pull(degs) %>% 
#     unique()
# })
# names(rayon_deg_gene_list) <- unique(esc_celltypes)
...
Schwann cells (Schwann-1 and Schwann–2)
oligodendrocyte precursor cells (OPCs) 
progenitors (Oligo Progen)
oligodendrocytes (Oligo-1 through Oligo-6)
microglia (Micro-1 through Micro-4 and Perivascular Micro)
macrophages

Astrocytes (WM Astro, GM Astro-1, and GM Astro-2). 
Meninges (Men-1 through Men-4). 

Vascular cells are shown in teal and include two groups of endothelial cells (Venous/Capillary Endo and Arterial Endo) 
pericytes/smooth muscle cells (Peri/SMC)
Ependymal cells 

Neurons  motoneurons (MN), 
excitatory dorsal neurons (ExDorsal), 
inhibitory dorsal neurons (InhDorsal), 
excitatory mid neurons (ExM), 
excitatory ventral neurons (EV), 
inhibitory mid neurons (InhM), 
...
```{r}
any(is.na(embryonic_sc$type_step2))
```
...
job::job({
  ...
  # save object
  saveRDS(merged_split_obj,"results/integrated_ref/embryonic_adult_spinal_cord_motor_cortex_integrated_normalized_seurat_obj_v2.RDS")
}, import = c("merged_split_obj"))
```
...
We next assigned putative neurotransmitter status to each population, identifying 19 glutamatergic populations (defined by the expression of SLC17A6) and 14 GABA/glycinergic populations (defined by expression of GAD1, GAD2, PAX2, and SLC6A5).

For AGE!
https://celltypes.brain-map.org/donors

```{r}
annots_table_clean <- top_level_table_merged %>% 
  mutate(class_label_standardized = case_when(
    number_of_cells < 50 | grepl("NULL", top_level_annotation)  ~ "remove",
    grepl("Astr|Endo|Blood|Epe|Ery|Hema|Lymph|Meso|Menin|Micro|Schwann|VLMC|OPC|Oligo|Pericytes", top_level_annotation) ~ "non-neuronal",
    ...
    top_level_standardized = case_when(
      ...
      grepl("Neuro", top_level_annotation) & grepl("Null", subtype_annotation) ~ "sc_neuron",
...
```{r}
annots_table_clean %>% 
  select(dataset, top_level_standardized, number_of_cells) %>% 
  group_by(dataset, top_level_standardized) %>% 
  summarize(num_cells = sum(number_of_cells)) %>% 
  ungroup() %>% 
  arrange(top_level_standardized, desc(num_cells)) %>% 
   View()
```
...
mportantly, the estimation of leverage scores only **requires data normalization**, can be computed efficiently for sparse datasets, and does not require any intensive computation or dimensional reduction steps.
...
meta_cols <- c("motorneuron", "age_clean", "donor", "tissue","top_level_annotation")
...
purrr::map(1:nrow(to_plot), function(x){
  ...
  outfile <- glue("figures/cell_refs/allen_yadav_radon_sketch/embryonic_adult_spinal_cord_motor_cortex_{gsub('umap.','', red)}_sketch_{col}_UMAP.pdf")
...
TO DO:
 - plot expression of the genes in the Yadav dorsal and ventral clusters 
 - plot SST cell-type expression 
 - Add Allen institute donor age 
 
```{r}
colors_mn <- c("Other"="grey80", "Motoneurons" = "#641A80FF", pMN = "#B63679FF" ,MN = "#FECE91FF")

reds <- c("umap.unintegrated", "umap.harmony", "umap.rpca")
meta_cols <- c("motorneuron", "age_clean", "donor", "tissue","top_level_annotation", "top_level_standardized")
...
purrr::map(1:nrow(to_plot), function(x){
  ...
  col <- to_plot[["meta_cols"]]
  print(col)
  outfile <- glue("figures/embryonic_adult_spinal_cord_motor_cortex_{gsub('umap.','', red)}_integrated_{col}_UMAP.pdf")
  cc <- ifelse(grepl("motorneuron", col), "colors_mn", "colors_vector")
  dp <- DimPlot(orig_obj,
        group.by = col,
        reduction = red,
...
```{r fig.height=10, fig.width=10}
bc <- orig_obj@meta.data %>% 
  filter(grepl("dorsal|ventral|mid|sst|motor", top_level_standardized)) %>% 
  pull(barcode)

neuron_subtypes <-  subset(orig_obj, cells = bc) 
