...
#Subset for highly variable genes only
# merged_scanvi_obj <- merged_scanvi_obj[merged_scanvi_obj@assays$RNA@var.features,]
...
# preserve counts
adata$X$shape  #87603, 34972
adata$layers["counts"] = adata$X$copy()
print(adata)

adata$var #34,972 rows x 6 columns
class(adata$var)
adata$var["vst.variable"]$value_counts()
...
# adata_subset <- sc$pp$highly_variable_genes(adata = adata, layer = "counts", subset = TRUE)
# adata_subset
```
...
adata$layers # no layers
adata$obs$head()
```

```{r}
scvi$settings$seed <- 20231030
