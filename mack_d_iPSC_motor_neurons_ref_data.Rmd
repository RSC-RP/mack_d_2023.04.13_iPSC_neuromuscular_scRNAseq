...
ImportError: /usr/lib/x86_64-linux-gnu/libssl.so.3: version `OPENSSL_3.2.0' not found (required by /home/jsmi26/miniforge3/envs/scanvi_scib/lib/python3.10/site-packages/h5py/../../.././libcurl.so.4)
      ...
      subtype_standardized = case_when(
        ...
        grepl("Neuro", top_level_annotation) &
          !grepl("Moto|^MN", subtype_annotation) ~ gsub("-[0-9]{1,2}$","", subtype_annotation) %>% 
                                                    gsub(".+\\s(.+)$", "\\1", .) %>% gsub("\\/|-","_", .),
        # grepl("Neuro", top_level_annotation) & grepl("Moto|^MN", subtype_annotation) & dataset == "Rayon" ~ "esc_mn",
        # grepl("Neuro", top_level_annotation) & grepl("Moto|^MN", subtype_annotation) & dataset == "Yadav" ~ "adult_mn",
...
celltype_annots <- read.csv("references/Allen_Rayon_Yadav_integrated_celltype_labels_table_CCedit.csv") %>% 
  ...
  mutate_at(vars(standardized_top_level_annotation), ~case_when(
    ...
    dataset == "Allen_M1" & top_level_annotation == "Micro-PVM" ~ 'Microglia',
    TRUE ~ .))
...
celltypes_to_retain <- celltype_annots %>% 
  ...
  mutate_at(vars(standardized_class_label), ~case_when(
    . == "Remove" & grepl("Neurons", top_level_annotation) ~ "Neuron",
    . == "Remove" & grepl("Micro-PVM", top_level_annotation) ~ "Glia",
    TRUE ~ gsub(" cells", "", .)))
...
plots2 <- purrr::map(unique(top_marker_genes$cluster), function(x){
    ...
    outname <- glue("figures/cell_refs/allen_yadav_rayon_integrated/subset/find_all_marker_genes/integrated_cell_ref_scanvi_subset_{x}_UMAP.pdf")
...
# write.csv(cell_type_table, "results/integrated_ref/Allen_Rayon_Yadav_integrated_cell_ref_subtype_standardized_table.csv", row.names = FALSE)
```
...
scanvi2_3d <- make_3D_umap(metadata = merged_split_obj@meta.data, 
             idents_column = c(paste0("umapscanvi2_",1:3), "subtype_standardized"), 
             extra_label_cols = c("subtype_standardized"), 
             selected_colors = colors_vector)
...
for_3d_plot <- merged_split_obj@meta.data %>% 
...
# pdf("figures/cell_refs/allen_yadav_rayon_integrated/subset/color_legend_subtype_standardized_3D_UMAP.pdf", height = 5, width = 10)
...
# pdf("figures/cell_refs/allen_yadav_rayon_integrated/subset/embryonic_adult_spinal_cord_motor_cortex_scanvi_integrated_subtype_standardized_plot3D_UMAP.pdf", height = 15, width = 15)
# png("figures/cell_refs/allen_yadav_rayon_integrated/subset/embryonic_adult_spinal_cord_motor_cortex_scanvi_integrated_subtype_standardized_plot3D_UMAP.png", height = 15, width = 15, units = "in", res = 150)
...
bc <- merged_split_obj@meta.data %>% 
  filter(grepl("motor|pMN", subtype_standardized)) %>% 
  rownames(.)
