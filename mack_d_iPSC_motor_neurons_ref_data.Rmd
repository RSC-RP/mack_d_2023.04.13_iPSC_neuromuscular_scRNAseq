...
# file.rename(from = file.path(outdir, "scib_results.svg"), to = file.path(outdir,"scib_benchmarker_batch.donor_subtype_standardized.svg"))

bm2$plot_results_table(min_max_scale=FALSE, save_dir = "results/integrated_ref/benchmark")
...
custom_dimplot <- function(metadata, colorby, dimred_colnames, custom_colors){
  ...
  labs_df <- metadata %>% 
  select(all_of(colorby), matches(xy_names)) %>% 
  group_by(!! colorby ) %>% 
  summarise(x = mean(!! x_names ), 
            y = mean(!! y_names))
...
```{r fig.height=10, fig.width=10}
labs_df <- merged_split_obj@meta.data %>% 
  select(subtype_standardized, matches("umapscanvi_[12]")) %>% 
  group_by(subtype_standardized) %>% 
  summarise(x = mean(umapscanvi_1), 
            y = mean(umapscanvi_2)) 


ggplot(merged_split_obj@meta.data, 
       aes(x = umapscanvi_1, y = umapscanvi_2, color = subtype_standardized)) +
  geom_point(alpha = 0.6) +
  ggrepel::geom_text_repel(data = labs_df, 
                           mapping = aes(x = x, y = y, label = subtype_standardized),
                           color = "black") +
  scale_color_manual(values = colors_vector) +
  guides(color = guide_legend(override.aes = list(alpha = 1))) +
  theme_classic() +
...
labs_df2 <- merged_split_obj@meta.data %>% 
  select(subtype_standardized, matches("umapscvi_[12]")) %>% 
  group_by(subtype_standardized) %>% 
  summarise(x = mean(umapscvi_1), 
            y = mean(umapscvi_2))

ggplot(merged_split_obj@meta.data, 
       aes(x = umapscvi_1, y = umapscvi_2, color = subtype_standardized)) +
  geom_point(alpha = 0.6) +
  ggrepel::geom_text_repel(data = labs_df2, 
                           mapping = aes(x = x, y = y, label = subtype_standardized),
                           color = "black") +
  scale_color_manual(values = colors_vector) +
  guides(color = guide_legend(override.aes = list(alpha = 1))) +
...
reds <- c("umap.unintegrated", "umap.harmony", "umap.rpca","umap.scvi","umap.scanvi")
meta_cols <- c("motorneuron", "age_clean", "donor", "tissue","top_level_annotation", "top_level_standardized", "class_label_standardized", "subtype_standardized")
subdir <- c("subset")
to_plot <- crossing(reds, meta_cols)
