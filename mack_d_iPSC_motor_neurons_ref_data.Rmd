...
Idents(embryonic_sc) <- embryonic_sc$type_step2
DimPlot(embryonic_sc,
        # # group.by = "type_step2",
        repel = TRUE,
        label.size = 4,
...
celltypes_to_retain <- celltype_annots %>% 
  filter(standardized_top_level_annotation != "Remove", !grepl("Ery", top_level_annotation), !grepl("Null", subtype_annotation)) %>% 
...
# table(celltypes_to_retain$standardized_subtype_annotation)
# sum(celltypes_to_retain$number_of_cells) #87,836
# any(is.na(celltypes_to_retain$top_level_annotation)) #FALSE

# table(celltypes_to_retain$top_level_annotation)
table(celltypes_to_retain$standardized_top_level_annotation)
```

```{r}
table(merged_split_obj$top_level_annotation %in% celltypes_to_retain$top_level_annotation)
```

```{r}
# subset_celltypes_obj@meta.data %>% 
#   group_by(top_level_annotation) %>% 
#   dplyr::count() %>% 
#   View()

subset_meta_data %>% 
  group_by(dataset,top_level_annotation, subtype_annotation) %>% 
  dplyr::count() %>% 
  View()
```
...
# Integration
integration_jobs <-  purrr::map(names(params)[4], function(param_set) {
...
For AGE! <https://celltypes.brain-map.org/donors>
- Donors not on there :/ 
...
```{r}
age_tissue_clean <- merged_split_obj@meta.data %>% 
...
merged_split_obj$tissue <- age_tissue_clean$tissue_clean
merged_split_obj$age_clean <- age_tissue_clean$age_clean

table(merged_split_obj$age_clean)
table(merged_split_obj$tissue)
```

```{r}
top_level_table_merged <- read.csv("results/integrated_ref/Allen_Rayon_Yadav_integrated_celltype_labels_table.csv")
```
...
```{r}
annots_table_clean <- top_level_table_merged %>% 
  mutate(class_label_standardized = case_when(
    grepl("Astr|Oligo", top_level_annotation) ~ "glia",
    grepl("OPC", top_level_annotation) ~ "glial_progenitor",
    grepl("Endo|Blood|Epe|Ery|Hema|Lymph|Meso|Menin|Micro|Schwann|VLMC|Pericytes", top_level_annotation) ~ "non-neuronal",
    ...
    top_level_standardized = case_when(
      ...
      grepl("Ependymal", top_level_annotation) ~ "glial",
      ...
      grepl("Micro-PVM", top_level_annotation) ~ "microglia",
  ...
  group_by(top_level_standardized) %>% 
  mutate(num_cells_std = sum(number_of_cells)) %>% 
  ungroup() %>% 
  mutate_at(vars(class_label_standardized:subtype_standardized), 
            ~ifelse(num_cells_std < 100 | grepl("NULL", top_level_annotation),  "remove", .))



# View(annots_table_clean)
```

```{r}
annots_table_clean %>% 
  filter(top_level_standardized == "remove")
```

```{r}
table(annots_table_clean$top_level_standardized)
# length(unique(annots_table_clean$top_level_standardized)) #33 (1 is remove tho) still too many for most plots
table(annots_table_clean$subtype_standardized) 
# table(annots_table_clean$subtype_standardized) %>% length # 52 OK for subclustering
table(annots_table_clean$class_label_standardized)
```

```{r}
# Update the metadata
updated_labels <- merged_split_obj@meta.data %>% 
  left_join(., select(annots_table_clean, -number_of_cells),
            by=c("top_level_annotation","subtype_annotation","dataset")) %>% 
  select(barcode, matches("standardized")) %>% 
  column_to_rownames("barcode")

head(updated_labels)
# dim(merged_split_obj@meta.data)
# dim(updated_labels)

# write.csv(updated_labels, "results/integrated_ref/updated_cell_type_labels.csv")

# merged_split_obj <- AddMetaData(merged_split_obj, updated_labels)
```


### Sketch Integration

mportantly, the estimation of leverage scores only **requires data
normalization**, can be computed efficiently for sparse datasets, and
does not require any intensive computation or dimensional reduction
steps.
...
```{r}
table(merged_split_obj$donor, merged_split_obj$age_clean)
```
...
purrr::map(1:nrow(to_plot), function(x){
  ...
  outfile <- glue("figures/cell_refs/allen_yadav_rayon_integrated/embryonic_adult_spinal_cord_motor_cortex_{gsub('umap.','', red)}_integrated_{col}_UMAP.pdf")
  cc <- ifelse(grepl("motorneuron", col), "colors_mn", "colors_vector")
...
```{r}
table(merged_split_obj)
```

```{r fig.height=10, fig.width=10}
bc <- merged_split_obj@meta.data %>% 
  filter(grepl("neuron|sst|motor", top_level_standardized)) %>% 
  pull(barcode)

neuron_subtypes <-  subset(merged_split_obj, cells = bc) 
# neuron_subtypes$subtype_annotation <- ifelse( grepl("Sst",neuron_subtypes$top_level_annotation), neuron_subtypes$top_level_annotation, gsub("\\-[0-9]{1,2}$", "", neuron_subtypes$subtype_annotation))
...
```{r}
table(neuron_subtypes$subtype_standardized, neuron_subtypes$top_level_standardized)
```
