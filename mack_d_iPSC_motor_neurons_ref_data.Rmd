...
# Integration
integration_jobs <-  purrr::map(names(params)[c(1,4)], function(param_set) {
...
```{r}
annots_table_clean <- merged_split_obj@meta.data %>% 
...
# Update the metadata
if(!all(colnames(annots_table_clean) %in% colnames(merged_split_obj@meta.data))){
  merged_split_obj <- AddMetaData(merged_split_obj, annots_table_clean)
}
```
