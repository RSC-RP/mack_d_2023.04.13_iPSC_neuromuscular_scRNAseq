...
```{r}
mc_azimuth_ref <- readRDS("resources/cell_refs/allen_institute/ref.Rds")

mc_azimuth_ref # 24,526 with 3000 features
# mc_azimuth_ref@meta.data
```
...
```{r}
ref_motor_cortex <- PercentageFeatureSet(ref_motor_cortex,
                               assay="RNA",
                               pattern = "^MT-",
                               col.name = "percent.mt")
...
ref_motor_cortex <- SCTransform(ref_motor_cortex, 
                                vst.flavor = "v2", 
                                method = "glmGamPoi",
                                verbose = T) %>%
