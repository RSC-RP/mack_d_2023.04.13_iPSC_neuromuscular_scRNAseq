...
mn_feature_plots <- purrr::map(c("umap.rpca","umap.harmony"), function(red){
  out1 <- glue("figures/cell_refs/allen_yadav_radon_integrated/embryonic_adult_spinal_cord_motor_cortex_integrated_{red}_MNgenes_UMAP.png")
  fp1 <- FeaturePlot(merged_split_obj,
                   label = TRUE, 
                   repel = TRUE,
                   label.size = 2,
                   features = rayon_marker_gene_list$rayon_Neuron_MN,
                   reduction = red) +
  patchwork::plot_annotation(title = "rayon_Neuron_MN")

  out2 <- glue("figures/cell_refs/allen_yadav_radon_integrated/embryonic_adult_spinal_cord_motor_cortex_integrated_{red}_MNgenes_subset_UMAP.png")
