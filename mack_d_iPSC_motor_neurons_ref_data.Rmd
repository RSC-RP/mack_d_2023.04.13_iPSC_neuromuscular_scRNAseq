...
# table(embryonic_sc_meta$dataset_id)
```
...
# saveRDS(embryonic_sc_counts,"resources/cell_refs/Rayon_2021/GSE171892_aggregated_postqc_seurat_obj_list.RDS")
```
...
```{r}
table(embryonic_sc$orig.ident)
dim(embryonic_sc_counts$human_CS14_thoracic_rep1)

CS14_thoracic_rep1_cts <- colSums(embryonic_sc_counts$human_CS14_thoracic_rep1[["RNA"]]@layers$counts)
table(CS14_thoracic_rep1_cts[order(CS14_thoracic_rep1_cts)] > 0)
```
...
saveRDS(embryonic_sc, "resources/cell_refs/Rayon_2021/GSE171892_aggregated_postqc_normalized_seurat_obj.RDS")
```
