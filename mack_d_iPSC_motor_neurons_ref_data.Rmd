...
make_3D_umap <- function(metadata, idents_column, extra_label_cols=NULL, selected_colors=NULL){
    ...
    # 
    # fig <- fig %>% plotly::layout(scene = list(xaxis=axx,yaxis=axy,zaxis=axz))
    # fig_cube <- fig %>% plotly::layout(scene = list(xaxis=axx,yaxis=axy,zaxis=axz, aspectmode='cube')) # To maintain cubic aspect
    return(fig)
}
```
...
dim(adata) # 87603 by 18,273 genes
...
pdf("figures/cell_refs/allen_yadav_rayon_integrated/subset/scanvi/scanvi_donor_subset_motorneuron_scanvi_UMAP.pdf", height = 7, width = 15)
sc$pl$umap(
...
```{r}
make_3D_umap(sobj_3d = merged_split_obj@meta.data, 
             idents_column = c(paste0("umaprpca_",1:3), "top_level_standardized"), 
...
alpha <- intersect(mn_subtype_markers$gene_name[mn_subtype_markers$celltype == "Alpha-MN"], 
                   rownames(merged_split_obj))
gamma <-  intersect(mn_subtype_markers$gene_name[mn_subtype_markers$celltype == "Gamma-MN"], 
                    rownames(merged_split_obj))

merged_split_obj <- ScaleData(merged_split_obj, features = c(gamma,alpha))
...
```{r fig.height=15, fig.width=15,message=FALSE, warning=FALSE}
Idents(merged_split_obj) <- merged_split_obj$subtype_standardized
mn_marker_list <- list("alpha" = alpha, "gamma" = gamma)

purrr::map(names(mn_marker_list), function(x){
    pdf(glue("figures/cell_refs/{x}_mn_marker_gene_scanvi.pdf"), height = 25, width = 25)
