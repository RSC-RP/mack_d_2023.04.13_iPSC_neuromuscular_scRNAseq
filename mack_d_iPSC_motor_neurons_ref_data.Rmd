...
tr_norm_mat <- t(as.matrix(GetAssayData(merged_scanvi_obj, layer = "data")))
...
norm_cts <- model$get_normalized_expression(library_size=10e4) #should this be 1e4?
latent <- model$get_latent_representation()

# unable to assign obsm in reticulate/anndataR in the same way as `adata$layers <- matrix()`. Limitation of the package currently. 
...
# scanvi_mde <- scvi$model$utils$mde(data = scanvi_latent) #an alternative to UMAP/TSNE 
...
```{r}
adata_update <- anndata::read_h5ad(filename = "results/integrated_ref/scanvi/embryonic_adult_spinal_cord_motor_cortex_scanvi_benchmarking_adata.h5ad")
...
adata_subset <- adata_update[grepl("motorneurons|pMN|astrocyte", adata_update$obs$subtype_standardized)]
# sc$pp$neighbors(adata_subset, use_rep='X_scANVI')
# sc$tl$umap(adata_subset, min_dist=0.3)

# pdf("figures/cell_refs/allen_yadav_rayon_integrated/subset/scanvi/scanvi_donor_subset_motorneuron_scanvi_UMAP.pdf", height = 10, width = 12)
sc$pl$umap(
    adata_subset,
    color=c('donor'),
    frameon=TRUE,
...
# benchmarking_df$Total
```

```{r fig.width=20}
bm1$plot_results_table(min_max_scale=FALSE)
```
...
adata_update <- anndata::read_h5ad(file.path(scanvi_dir, "embryonic_adult_spinal_cord_motor_cortex_scanvi_umap_mde_adata.h5ad"))
sc$tl$pca(adata_update, n_comps=30L, use_highly_variable=TRUE)
