...
```{r}
gobb <- clusterProfiler::read.gmt("resources/gene_sets/c5.go.bp.v2023.2.Hs.symbols.gmt")

c5.hpo <-  clusterProfiler::read.gmt("resources/gene_sets/c5.hpo.v2023.2.Hs.symbols.gmt")
```
...
https://docs.scvi-tools.org/en/stable/tutorials/notebooks/scrna/query_hlca_knn.html#loading-the-query-model-from-the-reference-files
...
predictions <- ipsc_adata$obs$subtype_standardized
table(predictions)
...
png("figures/day0_day50/cell_annotations/iPSC_motor_neuron_cell_annotations_scanvi_UMAP.png", height = 15, width = 15,units = "in", res = 125)
...
cc <- viridis::turbo(8) %>% 
  set_names(c("astrocyte","dorsal\ninterneuron 3","dorsal\ninterneuron 4", "microglia", "motor neuron", "neuroglial\nprogenitor", "oligodendrocytes", "FOO"))
cc <- cc[1:7]

# pdf("figures/day0_day50/cell_annotations/iPSC_motor_neuron_cell_annotations_scanvi_3D_UMAP_legend.pdf",   height = 5, width = 4)
par(mar = c(5,10,5,5))
barplot(rep(1,length(cc)), col = cc, names.arg = names(cc), 
        las = 2, horiz = TRUE, axes = FALSE)
# dev.off()
...
# png("figures/day0_day50/cell_annotations/iPSC_day0-day50_scanvi_celltype_annots_UMAP.png", height = 10, width = 10, units = "in", res = 125)
# pdf("figures/day0_day50/cell_annotations/iPSC_day0-day50_scanvi_celltype_annots_UMAP.pdf", height = 10, width = 10)
umap_scanvi_plot
# dev.off()
```

```{r}
purrr::map(unique(ipsc_obj@meta.data$time_point), function(x){
   p <- custom_dimplot(metadata = ipsc_obj@meta.data %>% 
                    filter(time_point == x),
               dimred_colnames = "umapscanvi",
               colorby = "subtype_labels",
               add_text = FALSE,
  ...
  # png(glue("figures/day0_day50/cell_annotations/iPSC_day{x}_scanvi_celltype_annots_UMAP.png"), height = 10, width = 10, units = "in", res = 125)
    pdf(glue("figures/day0_day50/cell_annotations/iPSC_day{x}_scanvi_celltype_annots_UMAP.pdf"), height = 10, width = 10)
  print(p)
  dev.off()
})
```
...
ipsc_obj$grps <- as.numeric(factor(ipsc_obj$subtype_labels, levels = names(cc)))

# pdf("figures/day0_day50/cell_annotations/iPSC_motor_neuron_cell_annotations_scanvi_3D_UMAP.pdf", height = 10, width = 12)
# png("figures/day0_day50/cell_annotations/iPSC_motor_neuron_cell_annotations_scanvi_3D_UMAP.png", height = 10, width = 12, res = 125, units = "in")
plot3D::scatter3D(x=ipsc_obj$umapscanvi_1, 
                  y=ipsc_obj$umapscanvi_2,
                  z=ipsc_obj$umapscanvi_3,
                   bty = "b2",
                  colvar=ipsc_obj$grps,
                  col=alpha(cc,  alpha = 0.25),
                  pch=19,
                  colkey=FALSE,
                  theta=t,
                  # phi=p,
...
# head(subtype_table)
subtype_table
# write.csv(subtype_table, "results/day0_day50/scanvi/iPSC_motor_neuron_day0-day50_time-course_scanvi_predictions_summary_table_v2.csv", row.names = FALSE)
```

```{r fig.height=10, fig.height=10}
# total_cells_plot <- ggplot(subtype_table, 
#                            aes(x = time_point, y = number_of_cells, group = subtype_standardized)) +
...
pct_cells_plot <- ggplot(subtype_table, aes(x = time_point, y = percent_of_cells, group = subtype_labels)) +
  geom_point(aes(color = subtype_labels), size = 3, alpha=0.7) +
  geom_line(aes(color = subtype_labels), linewidth = 0.5, linetype = "dashed") +
...
# pdf("figures/day0_day50/cell_annotations/iPSC_motor_neuron_celltypes_per_timepoint_lineplot.pdf", height = 5, width = 8)
pct_cells_plot
# dev.off()
```
...
```{r}
DefaultAssay(ipsc_obj) <- "SCT"
ipsc_obj <- FindNeighbors(ipsc_obj,
                          reduction = "scanvi", 
                          dims = 1:30, 
                          k.param = 20,
...
ipsc_obj <- FindSubCluster(ipsc_obj, 
                           cluster = "motorneurons", 
                           subcluster.name = "motorneurons_clusters",
                           graph.name = "SCANVI_snn",
                           resolution = 0.1) %>% 
            FindSubCluster(cluster = "progenitor", 
                           subcluster.name = "progenitor_clusters",
                           graph.name = "SCANVI_snn", 
                           resolution = 0.25)


ipsc_obj@meta.data %>% 
  filter(grepl("day27", orig.ident)) %>% 
  select(1:2, subtype_standardized, motorneurons_clusters, progenitor_clusters) %>% 
  head()

# table(ipsc_obj$motorneurons_clusters)
# table(ipsc_obj$progenitor_clusters)

# Add the metadata
mn_prog_subclusts <- ipsc_obj@meta.data %>% 
  mutate(subtype_standardized_subclusters = case_when(
   subtype_standardized == "progenitor" ~ progenitor_clusters,
   subtype_standardized == "motorneurons" ~ motorneurons_clusters,
   TRUE ~ subtype_standardized
  )) %>% 
  pull(subtype_standardized_subclusters, name = barcode)
  
# table(mn_prog_subclusts)
ipsc_obj <- AddMetaData(ipsc_obj, metadata = mn_prog_subclusts, col.name = "subtype_standardized_subclusters")
length(unique(ipsc_obj$subtype_standardized_subclusters)) #19
table(ipsc_obj$subtype_standardized_subclusters)

job::job({
  saveRDS(ipsc_obj,"results/day0_day50/seurat/iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_anno_seurat_obj_v5.RDS")
}, import = c("ipsc_obj"))
```
...
subcluster_3d <-   make_3D_umap(metadata = ipsc_obj@meta.data,
              idents_column = c(paste0("umapscanvi", "_",1:3)),
...
write.csv(subclusters_results,"results/day0_day50/degs/iPSC_celltypes_subclusters_results_sct_MAST_DEGs.csv", row.names = FALSE)

celltypes_results <- make_degs_df(degs_celltypes)
# write.csv(celltypes_results,"results/day0_day50/degs/iPSC_celltypes_sct_MAST_DEGs.csv", row.names = FALSE)
```

```{r}
celltypes_results <- read.csv("results/day0_day50/degs/iPSC_celltypes_sct_MAST_DEGs.csv")
celltypes_results
dim(celltypes_results)
```

```{r}
top10 <- celltypes_results %>% 
  mutate(diff = pct.1 - pct.2) %>% 
  group_by(cluster) %>% 
  dplyr::top_n(n = 10, wt = avg_log2FC) %>% 
  ungroup()

# write.csv(top10, "results/day0_day50/degs/iPSC_celltypes_sct_MAST_DEGs.csv", row.names = FALSE)
```
...
```{r}
day0_meta <- ipsc_obj@meta.data %>% 
  filter(time_point == 0)

ref_cells <- day0_meta %>% 
  filter(subtype_standardized == "progenitor") %>% 
  pull(barcode)

dl3_cells <- day0_meta %>% 
  filter(subtype_standardized == "dl3") %>% 
  pull(barcode)
...
table(day0_meta$subtype_standardized)
```


```{r}
Idents(ipsc_obj) <- ipsc_obj$orig.ident
check_annots <- c("dl3", "microglia")

day0_DE <- purrr::map(1:2, function(i){
  comp_group <- check_annots[i]
  print(comp_group)
  FindMarkers(object = ipsc_obj, 
              ident.1 = comp_group,
              ident.2 = "progenitor",
              subset.ident = "iPSC_ipsc",
...
GeneCards Summary for BICD1 Gene
BICD1 (BICD Cargo Adaptor 1) is a Protein Coding gene. Diseases associated with BICD1 include Spinal Muscular Atrophy With Lower Extremity Predominant and Central Corneal Ulcer.

```{r}
day0_DE$microglia$cluster <- "microglia"
microglia <- make_degs_df(rownames_to_column(day0_DE$microglia, "gene"))

microglia
# write.csv(microglia, "results/day0_day50/scanvi/iPSC_motor_neuron_day0_MicrogliavsProgenitor_DEGs.csv", row.names = FALSE)
```


```{r}
day0_DE$dl3$cluster <- "dl3"

dl3 <- make_degs_df(rownames_to_column(day0_DE$dl3, "gene")) %>% 
  left_join(.,ipsc_obj[["RNA"]]@meta.data, by = c("gene"="gene_name_fix"))

head(dl3)
# write.csv(dl3, "results/day0_day50/scanvi/iPSC_motor_neuron_day0_DL3vsProgenitor_DEGs.csv", row.names = FALSE)
```
...
dl3_gsea_res <- GSEA(geneList = pull(in_genes, avg_log2FC, name = gene_name),
                            pvalueCutoff = 0.05, 
                            TERM2GENE = bind_rows(gobb, c5.hpo),
                            seed = seed_val)@result 

dl3_gsea_res <- dl3_gsea_res %>% 
  janitor::clean_names() %>% 
  select(id,nes,p_adjust, set_size,
         everything())

dl3_gsea_res
# write.csv(dl3_gsea_res,"results/day0_day50/scanvi/iPSC_motor_neuron_day0_DL3vsProgenitor_GSEA.csv")
```



```{r fig.height = 7, width = 10}
Idents(ipsc_obj) <- ipsc_obj$orig.ident
VlnPlot(object = ipsc_obj,
            features = marker_genes_iPSC$iPSCs,
            group.by = "subtype_standardized",
            idents = "iPSC_ipsc", 
        assay = "SCT",
        layer = "data")
...
VlnPlot(object = ipsc_obj,
            features = marker_genes_iPSC$MNPCs,
            group.by = "subtype_standardized",
            idents = "iPSC_ipsc", 
        assay = "SCT",
        layer = "data")
...
VlnPlot(object = ipsc_obj,
            features = marker_genes_iPSC$INs,
            group.by = "subtype_standardized",
            idents = "iPSC_ipsc", 
        assay = "SCT",
        layer = "data")
...
```{r}
dorsal_IN_markers <- unlist(rayon_marker_gene_list[grep("rayon_Neuron_dl", names(rayon_marker_gene_list))]) %>% 
  unique() %>% 
  base::intersect(., rownames(ipsc_obj[["SCT"]]@data))

length(dorsal_IN_markers)
```

```{r}
dl3_upreg <- dl3 %>% 
  filter(avg_log2FC > 0, p_val_adj < 0.001)


dl3_upreg_expn_df <- ipsc_obj[["SCT"]]@data[dl3_upreg$gene,c(dl3_cells, ref_cells)] %>% 
  as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  pivot_longer(cols = -gene) %>% 
  left_join(., day0_meta, by = c("name"="barcode"))

head(dl3_upreg_expn_df)
quantile(dl3_upreg_expn_df$value, probs = c(0.5, 0.75, 0.9, 0.95, 1))
```



```{r fig.height=10, fig.width=15}
ggplot(dl3_upreg_expn_df, aes(x = subtype_standardized, y = value)) +
  geom_boxplot(aes(fill = subtype_standardized),
               outlier.colour = NA) +
  theme_classic()
```
...
```{r fig.height = 7, width = 10}
VlnPlot(object = ipsc_obj,
            features = rayon_marker_gene_list$rayon_Neuron_dl3,
            group.by = "subtype_standardized",
            idents = "iPSC_ipsc", 
        assay = "SCT",
        layer = "data")
...
VlnPlot(object = ipsc_obj,
            features = rayon_marker_gene_list$rayon_Progenitor_Null_Progenitor,
            group.by = "subtype_standardized",
            idents = "iPSC_ipsc", 
        assay = "SCT",
        layer = "data")
...
mat <- ipsc_obj[["SCT"]]@data[dl3$gene,c(dl3_cells, ref_cells)]
mat <- as.matrix(mat)
dim(mat)


colors <- colorRampPalette(c(paste("dodgerblue",4:1), "white",paste("red",1:4)))(299)
pheatmap::pheatmap(mat = mat[in_genes$gene, ],
                   annotation_col = select(day0_meta,
                                           id, subtype_standardized),
                   scale = "row",
                   color = colors, 
