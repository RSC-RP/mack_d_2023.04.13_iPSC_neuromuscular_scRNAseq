...
ipsc_obj <- CellCycleScoring(ipsc_obj, 
                             # assay = "RNA",
                             s.features = cc_genes$s.genes, 
                             g2m.features = cc_genes$g2m.genes,
                             set.ident = FALSE)
ipsc_obj$CC.Difference <- ipsc_obj$S.Score - ipsc_obj$G2M.Score
FeaturePlot(ipsc_obj, reduction = "umap", features = "CC.Difference")
...
scanvi_norm_mat <- scanvi_query$get_normalized_expression(n_samples = 25L,
                                                          transform_batch = levels(ipsc_obj$id)[1:11],
                                                          library_size = 1e4)
scanvi_norm_mat <- py_to_r(scanvi_norm_mat)
scanvi_norm_mat[1:5,1:5]
...
```{r}
res <- c(0.6)
ipsc_obj <- FindClusters(ipsc_obj,
                         resolution = res,
                         cluster.name = glue("rpca_snn_res{res}"),
                         graph.name = "rpca_snn")
```
...
# DimPlot(ipsc_obj, 
#         reduction = "umap.rpca",
#         group.by = "subtype_standard_detail",
#         label = TRUE,
#         label.size = 2,
#         repel = TRUE,
#         dims = c(1,2)) +
...
p1 <- make_3D_umap(metadata = ipsc_obj@meta.data,
             ...
             label_cols = c("subtype_standardized","id","batch","subtype_standard_detail"), 
...
```{r}
table(ipsc_obj$subtype_standardized_subclusters)[order(names(table(ipsc_obj$subtype_standardized_subclusters)))]
```
Note: make another celltype line plot vs time with the subclusters 
- these might 
