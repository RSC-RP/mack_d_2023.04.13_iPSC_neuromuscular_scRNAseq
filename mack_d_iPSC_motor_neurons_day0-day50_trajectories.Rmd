...
tmk$write_to_adata()
anndata::write_h5ad(anndata = tmk$adata,
                    filename = "results/day0_day50/cellrank/iPSC_MNs_day0-day50_moscot_cellrank_RealTimeKernel.h5ad")
```
...
```{r}
# corrplot::corrplot(corr = transition_mat)
```
...
gpcca2$compute_schur()

gpcca2$plot_spectrum(real_only=TRUE,
...
meta %>% 
  ...
  select(macrostates_fwd, term_states_fwd, init_states_fwd, umapscanvi_1:init_states_fwd_probs) %>% 
  group_by(macrostates_fwd, term_states_fwd, init_states_fwd) %>% 
  dplyr::count()
```
...
```{r fig.height=10, fig.width=10}
gpcca$plot_coarse_T(save = here::here("figures/gpcca_plot_coarse_T.pdf"), 
                     figsize = c(10L,10L), dpi = 125L)


gpcca$plot_macrostate_composition(key="subtype_standardized",
                                  save = here::here("figures/gpcca_plot_macrostate_composition.pdf"),
