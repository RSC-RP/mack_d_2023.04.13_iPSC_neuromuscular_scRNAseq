...
merged_norm_obj@meta.data$log2_nCount_RNA <- log2(merged_norm_obj@meta.data$nCount_RNA)
...
merged_norm_obj[["RNA"]] <- split(merged_norm_obj[["RNA"]],
                                  f = merged_norm_obj$orig.ident)
merged_norm_obj <- NormalizeData(merged_norm_obj,
...
merged_norm_obj[["RNA"]] <- JoinLayers(merged_norm_obj[["RNA"]])
```

```{r}
# find clusters with harmoning integrated UMAP
DefaultAssay(merged_norm_obj) <- "SCT"
...
```{r}
table(merged_norm_obj$harmony_snn_res.0.7)
```

## Visualization 

```{r}
table(merged_norm_obj$subtype_standardized)
```

```{r fig.height=10, fig.width=10}
custom_dimplot(metadata = merged_norm_obj@meta.data, 
               colorby = "batch", 
...
custom_dimplot(metadata = merged_norm_obj@meta.data, 
               colorby = "harmony_snn_res.0.7", 
               dimred_colnames = "umapharmony", 
               custom_colors = ggpubr::get_palette("simpsons", 24))
```

## Investigate Outliers 

```{r}
outliers <- subset(merged_norm_obj, subset = umapharmony_2 > 8)

outliers
table(outliers$harmony_snn_res.0.7)
```
...
```{r}
# quantile(merged_norm_obj$log2_nCount_RNA, probs = seq(0.5,1, length.out = 11))
FeaturePlot(merged_norm_obj,
...
```{r}
outliers$orig.ident %>% table()
```

```{r}
DefaultAssay(outliers_filt) <- "RNA"

# outliers_filt[["RNA"]] <- split(outliers_filt[["RNA"]], f = outliers_filt$orig.ident)

umi_counts_c20 <-  sapply(grep("counts",Layers(outliers_filt@assays$RNA), value = TRUE), function(x) {
   colSums(outliers_filt@assays$RNA[[x]] > 0)
})
...
iPSC_obj <- subset(merged_norm_obj, subset =  umapharmony_2 < 8)
...
iPSC_obj <- iPSC_obj %>% 
    FindNeighbors(reduction = "harmony",
                dims = dims, 
                k.param = 20, #default
                # graph.name = c("subset_harmony_nn","subset_harmony_snn")
                ) %>% 
    FindClusters(resolution = 0.7, 
                 algorithm = 3, 
                 # graph.name = "subset_harmony_snn", 
...
iPSC_obj
iPSC_obj <- AddMetaData(iPSC_obj, 
                        metadata = iPSC_obj@reductions$umap_harmony_subset@cell.embeddings)

table(iPSC_obj$subset_harmony_clusters) # 22 clusters 
```
...
```{r}
sce <- slingshot(sce, 
                 clusterLabels = 'subset_harmony_clusters',
                 reducedDim = 'PCA')
```
