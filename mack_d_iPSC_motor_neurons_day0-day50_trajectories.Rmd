...
tmk # Using automatic `threshold=3.37183244427879e-33`
...
# transition_mat[1:5,1:5]
# dimnames(transition_mat) # cell ids
# "dgRMatrix"
# attr(,"package")
# [1] "Matrix"
```
...
```{r}
tmk$obs$subtype_standardized$value_counts()
```
...
# gpcca$set_terminal_states(states = c("astrocyte", "dl4", "microglia",'motorneurons_1', 'motorneurons_2', 'motorneurons_3', 'motorneurons_4', 'progenitor_1', 'progenitor_2', 'progenitor_3', 'progenitor_4'))
```
...
```{r}
# gpcca2 <- cr$estimators$GPCCA(tmk) # this works
# 
# gpcca2$compute_schur(n_components=20,
#                     verbose = TRUE) # gives an errror?
# 
# gpcca2$plot_spectrum()
# gpcca2$compute_macrostates(n_states=11,
...
Adding `adata.uns['eigendecomposition_fwd']`
       ...
       `.eigendecomposition`
    Finish (0:00:59)
Calculating minChi criterion in interval `[6, 15]`
...
How is the goodness of fit for the number of macrostates selected? 
...
gpcca$predict_initial_states(allow_overlap = TRUE,
                             n_states = 1) # 30 cells overlap in terminal and initial status
...
check
# table(check$init_states_fwd) # DL3 SHOULD NOT BE THE initial STATE
filter(check, !is.na(init_states_fwd))
```
```{r}
check %>% 
  select(term_states_fwd_probs,init_states_fwd_probs) %>% 
...
```{r fig.height=10, fig.width=10}
gpcca$plot_fate_probabilities(states = c('dl3',paste0('motorneurons_',1:4)),
                              same_plot=FALSE)
```
