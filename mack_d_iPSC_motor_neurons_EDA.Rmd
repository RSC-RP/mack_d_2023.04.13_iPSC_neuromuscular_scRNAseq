...
current_merge_obj <- 
  "iPSC_motor_neuron_time-course_SCT_UMAP_LeidenClust_Seurat_obj.RDS"
...
est_first_pass <- purrr::map(names(soup_channels), function(x){
  ...
  min <- 1
  sc <- soup_channels[[x]]
  outfile2 <- glue("figures/soupX/{x}_autoEstCont_figure.pdf")
  tryCatch(expr = {
...
save_corrected_counts <- purrr::map(names(final_estimates), function(x){
  outdir <- file.path("results/seurat/soupX_counts", x)
  print(outdir)
...
```{r eval=FALSE}
job::job({
  saveRDS(final_estimates, "results/seurat/soupX_counts/mack_iPSC_MN_soupX_channel_objs_list.RDS")
