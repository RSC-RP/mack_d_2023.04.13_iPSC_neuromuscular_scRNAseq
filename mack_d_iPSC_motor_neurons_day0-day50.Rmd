...
library(Seurat)
# library(scDblFinder)
# ERROR: dependencies ‘scran’, ‘scater’, ‘scuttle’, ‘xgboost’ are not available for package ‘scDblFinder’
library(SoupX)
```
...
custom_dimplot <- function(metadata, colorby, dimred_colnames, custom_colors,
  ...
  labs_df <- metadata %>%
   ...
   if(is.character(nx)){
     x_val <-  ifelse(grepl(nx, labs_df[[colorby]]), 1.5, 0)
   }else{
     x_val <- nx
   }
...
```{r}
features <- data.table::fread("resources/Day11/filtered_feature_bc_matrix_day11/features.tsv.gz", header = FALSE) %>% 
  rename_all(~c("ensembl_id","gene_name", "type")) %>% 
  mutate(index = 1:nrow(.)) %>% 
  group_by(gene_name) %>% 
  mutate(gene_name_fix=case_when(
    n() == 1 ~ gene_name, 
    n() > 1 ~ paste(gene_name,0:c(n()-1), sep=".")
  )) %>% 
  mutate_at(vars(gene_name_fix), ~case_when(
    n() > 1 ~ gsub("\\.0$", "", .),
    TRUE ~ .
  )) %>% 
  ungroup() %>% 
  mutate_at(vars(gene_name_fix), ~ifelse(is.na(gene_name), gene_id, .)) %>% 
  mutate_at(vars(gene_name_fix), ~gsub("_","-", .)) %>% 
  as.data.frame() %>% 
...
head(features)
dim(features)
# write.csv(features,"results/filtered_feature_bc_features.csv")
# table(features$V3)
```
...
```{r}
doublet_scores <- purrr::map_dfr(dir("results/doublet_scores", 
                                     pattern = "_scDblFinder_scores.csv",
...
```{r}
pct_doubles_df <- doublet_scores %>% 
    select(orig.ident,barcode,
         distanceToNearest,score,
         class,matches("^ratio")) %>% 
  mutate(id = factor(orig.ident, 
                     levels = unique(orig.ident))) %>% 
  group_by(id) %>% 
  mutate(total_cells = n()) %>% 
  ungroup() %>% 
  group_by(id, total_cells, class) %>% 
  dplyr::count(name = "number_of_cells") %>% 
  ungroup() %>% 
  mutate(percent_of_cells = round(number_of_cells / total_cells * 100, digits = 1))

head(pct_doubles_df)
# dim(pct_doubles_df)
# write.csv(pct_doubles_df, "results/doublet_scores/mack_iPSC_MN_day0-day50_rerun_scDblFinder_percent_doublets.csv", row.names = FALSE)
```
...
if(!exists("merged_obj")){
  merged_obj <- readRDS("results/day0_day50/seurat/iPSC_motor_neuron_day0-day50_time-course_SCT_UMAP_Seurat_obj.RDS")
}
# Examine the upper nCount_RNA limits possible. Day5 and Day8 appear to have very low thresholds that are too agressive. 
merged_obj$mt.threshold <- ifelse(merged_obj$percent.mt > 8, "remove", "keep")

lib_size_thr <- 5e4
merged_obj$lib_size_outlier <-  merged_obj$nCount_RNA >  lib_size_thr # 933 cells are extreme lib size outliers
merged_obj$id <- factor(merged_obj$orig.ident, levels = unique(merged_obj$orig.ident))

# upper limit for all samples is too low/ too aggressive for some samples
all_objs_upl <- scuttle::isOutlier(merged_obj$nCount_RNA,
                                     type="higher", 
                                     nmads = 3) #30,157
```

```{r}
ncount_hist <- ggplot(merged_obj@meta.data, aes(x = nCount_RNA)) +
  geom_histogram(binwidth = 500) +
  scale_x_continuous(breaks = seq(0, max(merged_obj@meta.data$nCount_RNA), by= 10000)) +
  geom_vline(xintercept = c(lib_size_thr), color = "red", linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 35, vjust = 1, hjust = 1)) +
  theme_bw()

ncount_threshold <- ggplot(merged_obj@meta.data, 
       aes(x = id, y = nCount_RNA)) +
  geom_point(aes(color = mt.threshold),
             alpha = 0.75, size = 0.5, position = position_jitter()) +
  geom_violin(fill = "blue", alpha = 0.2)  +
  geom_hline(yintercept = c(lib_size_thr), color = "black", linetype = "dashed") +
  scale_color_brewer(palette = "Set1") +
  theme(axis.text.x = element_text(angle = 35, vjust = 1, hjust = 1)) +
  theme_bw()

nfeature_boxplot <- ggplot(merged_obj@meta.data, 
       aes(x = id, y = nFeature_RNA)) +
  geom_point(aes(color = mt.threshold),
             alpha = 0.75, size = 0.5, position = position_jitter()) +
  geom_violin(fill = "blue", alpha = 0.2)  +
  scale_color_brewer(palette = "Set1") +
  geom_hline(yintercept = c(10461, 200), color = "black", linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 35, vjust = 1, hjust = 1)) +
  theme_bw()


# png("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_nCount_RNA_boxplot.png", height = 10, width = 20, res = 125, units = "in")
# ncount_threshold
# dev.off()
# 
# pdf("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_nCount_RNA_histogram.pdf", height = 7, width = 10)
# ncount_hist
# dev.off()
# 
# pdf("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_nFeature_RNA_boxplot.pdf", height = 10, width = 20)
# nfeature_boxplot
# dev.off()
```

```{r eval=TRUE}
if(!exists("seurat_objs_dbls_list")){
  seurat_objs_dbls_list <- purrr::map(seurat_objs_dbls, function(x) readRDS(x))
}

# job::job({
  # loop through each sample and create dynamic filting thresholds
  filter_objs <- purrr::map(names(seurat_objs_dbls_list), function(x){
        ...
        if( any(!file.exists(outfile) | !file.exists(out_summary)) ){
          ...
          #  at least 500 reads per cell
          #  less than 50,000 total reads per cell
          rna_upper <- 5e4
          rna_lower <- 500
          feat_lower <- 200
          ...
          ths <-  data.frame(sample=x,
                           mito_th=mito_th,
                           rb_th=rb_th,
                           ncount_rna_th1=rna_upper,
                           ncount_rna_th2=rna_lower,
                           nfeat_rna_th=feat_lower,
  ...
  names(filter_objs) <- paste0("iPSC_", names(seurat_objs_dbls_list))
  
# }, import = c("seurat_objs_dbls_list"))
```
...
threshold_summary <- purrr::map_dfr(outpaths, function(x){
  ...
  mutate(sample_id = paste0("iPSC_", sample), 
         percent = round(output_number_cells / input_number_cells * 100, digits = 1)) %>%
...
custom_violin_qc <- function(metadata, threshold_summary){
  
  df <- metadata %>% 
    rownames_to_column("barcode") %>% 
    select(barcode:percent.ribo, class) %>% 
    mutate(class = as.numeric(as.factor(class))) %>% 
    left_join(., select(threshold_summary,sample_id:batch),
              by = c("orig.ident"="sample_id")) %>%
    mutate(ribo_rm = percent.ribo < rb_th,
           mito_rm = percent.mt > mito_th,
           nCount_RNA_rm = nCount_RNA < 500 | nCount_RNA > 5e4,
           nFeature_RNA_rm = nFeature_RNA < 200,
           doublet = class == 1)
  
  values_df <-   df %>% 
    select(orig.ident, barcode, nCount_RNA:class) %>% 
    pivot_longer(cols = nCount_RNA:class,
                 names_to = "metric",
                 values_to = "value") %>% 
    mutate(type = case_when(
      grepl("nCount", metric) ~ "nCount_RNA", 
      grepl("nFeat", metric) ~ "nFeature_RNA", 
      grepl("percent.mt", metric) ~ "percent.mt", 
      grepl("ribo", metric) ~ "percent.ribo", 
      grepl("class", metric) ~ "doublet"
      ))
  
  categories_df <- df %>% 
    select(barcode, ribo_rm:doublet) %>% 
    pivot_longer(cols = ribo_rm:doublet,
                 names_to = "category",
                 values_to = "remove_cell") %>% 
    mutate(type = case_when(
      grepl("nCount", category) ~ "nCount_RNA",
      grepl("nFeat", category) ~ "nFeature_RNA", 
      grepl("mito", category) ~ "percent.mt", 
      grepl("ribo", category) ~ "percent.ribo",
      grepl("doublet", category) ~ category,
    ))
  
  input <- values_df %>% 
    left_join(categories_df, by = c("barcode", "type")) 

  plot <- ggplot(input, aes(x = type, y = value, color = remove_cell)) +
      geom_point(alpha = 0.75, size = 0.5,
                 position = position_jitter()) +
      geom_violin(color = "grey", alpha = 0.2)  +
      facet_wrap(~type, scales = 'free', ncol = 5) +
      theme_bw() 

}

# pdf("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_filtered_seurat_violins.pdf", height = 8, width = 10)

# dev.off()
```

```{r}
# pdf("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_filtered_mito_thresholds.pdf", height = 8, width = 10)
...
```{r}
threshold_summary_update <- read.csv("results/day0_day50/seurat/filtering_stats/mack_iPSC_MN_day0-day50_cell_filtering_final_threshold_summary.csv")
```
```{r}
sum(threshold_summary_update$input_number_cells) # 98,532
sum(threshold_summary_update$output_number_cells) # 86,954
```
...
# write.csv(est_df_subset, "results/soupX_counts/mack_iPSC_MN_day0-day50_soupX_contamination_estimates_per_sample.csv", row.names = FALSE)
...
soupx_objs <- purrr::map(outs, function(res){
  ...
  counts <- Read10X(res)
  sample_id <- basename(res)
  print(glue("{sample_id}"))
  
  mdata <- metadata %>% 
    filter(orig.ident == sample_id) %>% 
    mutate(orig_obarcode = gsub("^iPSC_[A-Za-z].+_","", barcode)) %>% 
    filter(orig_obarcode %in% base::intersect(colnames(counts), orig_obarcode)) %>% 
    set_rownames(.$orig_obarcode) %>% 
    as.data.frame()
...
# An object of class Seurat 
# 33538 features across 86,954 samples within 1 assay 
# Active assay: RNA (33538 features, 0 variable features)
merged_filt_obj 
...
dim(merged_filt_obj) #86,954 cells
...
if(!exists("merged_norm_obj")){
  merged_norm_obj <- readRDS("results/day0_day50/seurat/non_integrated/iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_seurat_obj_v5.RDS")
}
...
merged_norm_obj #60470 features across 86,954 samples within 2 assays 
...
job::job({
  ...
  # calculate cell cycle scores on normalized data
  merged_norm_obj <- CellCycleScoring(merged_norm_obj, 
                               assay = "SCT",
                               s.features = cc_genes$s.genes, 
                               g2m.features = cc_genes$g2m.genes,
                               set.ident = FALSE)
  
  # add cell cycle difference 
  ...
  merged_norm_obj$cc.difference <- merged_norm_obj$S.Score - merged_norm_obj$G2M.Score
  
  # overwrite the old SCT assay and replace with this new one
  merged_norm_cc_obj <- SCTransform(merged_norm_obj,
                             assay="RNA",
                             new.assay.name = "SCT",
  ...
  # re-run down-stream processing with the new pearson residuals that have CC differences regressed out. 
  merged_norm_cc_obj <-   RunPCA(merged_norm_cc_obj,
                                assay = "SCT",
                                npcs = 50,
                                verbose = TRUE) %>% 
          RunUMAP(reduction = "pca",
  ...
  merged_norm_cc_obj <- merged_norm_cc_obj %>%
          FindNeighbors(reduction = "pca",
                        dims = 1:50,
                        k.param = 20,
                        nn.method = "annoy",
  ...
  # Add the new UMAP dims
  merged_norm_cc_obj <- AddMetaData(merged_norm_cc_obj,
                                    metadata = merged_norm_cc_obj@reductions$umap@cell.embeddings)
  
    saveRDS(merged_norm_cc_obj, glue("results/day0_day50/seurat/iPSC_MN_day0-day50_time-course_filtered_merged_soupX_normalized_cellcycle_seurat_obj_v5.RDS"))

}, import = c("merged_norm_obj"))
```

```{r fig.height=10, fig.width=10}
if(!exists("merged_norm_cc_obj")){
  merged_norm_cc_obj <- readRDS("results/day0_day50/seurat/non_integrated/iPSC_MN_day0-day50_time-course_filtered_merged_soupX_normalized_cellcycle_seurat_obj_v5.RDS")
}

DimPlot(merged_norm_cc_obj, 
        reduction = "umap",
        group.by = "time_point")

FeaturePlot(merged_norm_cc_obj, 
            reduction = "umap", 
            features = "S.Score")
...
```{r}
cc_genes <- Seurat::cc.genes.updated.2019
chk <- RunPCA(merged_norm_cc_obj, features = c(cc_genes$s.genes,
                                               cc_genes$g2m.genes))
DimPlot(chk,  reduction = "pca", group.by = "Phase")
```


```{r fig.width=10}
 custom_dimplot(metadata = merged_norm_cc_obj@meta.data,
                         colorby = "batch", 
...
```{r}
cc_3D_umap_batch <-  make_3D_umap(metadata = merged_norm_cc_obj@meta.data %>% 
               filter(time_point > 30), 
             idents_column = c(paste0("umap_",1:3)), 
             label_cols = c("id","batch"), 
             selected_colors = sample_id_colors)

# htmlwidgets::saveWidget(as_widget(cc_3D_umap_batch), "figures/day0_day50/filtered_merged_results/iPSC_cell_cycle_regression_batch_UMAP.html")
...
# head(genes_detected)
# dim(genes_detected)
# write.csv(genes_detected, "results/day0_day50/seurat/filtering_stats/mack_iPSC_MN_day0-day50_number_genes_detected_per_sample.csv", row.names = FALSE)
```

```{r fig.height=10}
meta <- merged_norm_obj@meta.data %>% 
  mutate(nCount_RNA_lims = ifelse(nCount_RNA > quantile(nCount_RNA, probs = c(0.95)), 
                                  quantile(nCount_RNA, probs = c(0.95)), 
                                  nCount_RNA))
...
median(unlist(umi_counts_filt)) # median genes detected across all cells
mean(unlist(umi_counts_filt)) # mean genes detected across all cells
...
for(VERSION in versions){
  ...
  temp <- merged_filt_obj[grep("^MT-", rownames(merged_filt_obj), invert = TRUE), ]
  bc <- temp@meta.data %>% 
    filter(batch != "batch3" | time_point == 30) %>%
    pull(barcode) 
  
  temp <- subset(temp, cells = bc)
  print(table(temp$batch))
  ...
  #regularized negative binomial regression to normalize UMI count data
  #Note that this single command replaces NormalizeData(), ScaleData(), and FindVariableFeatures().
  merged_subset_obj <- SCTransform(temp,
                           assay="RNA",
                           new.assay.name = "SCT",
                           vars.to.regress = "percent.mt",
                           method = "glmGamPoi",
  ...
  merged_subset_obj <-   RunPCA(merged_subset_obj,
                              assay = "SCT",
                              npcs = 50,
                              verbose = TRUE) %>% 
        RunUMAP(reduction = "pca",
    ...
    merged_subset_obj <- merged_subset_obj %>%
        FindNeighbors(reduction = "pca",
                      dims = 1:50,
                      k.param = 20,
                      nn.method = "annoy",
    ...
    merged_subset_obj <- AddMetaData(merged_subset_obj,
                                   as.data.frame(merged_subset_obj@reductions$umap@cell.embeddings))
    
    job::job({
          saveRDS(merged_subset_obj, glue("results/day0_day50/seurat/iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_B124_seurat_obj_{VERSION}.RDS"))
    }, import = c("merged_subset_obj"))
}
rm(merged_filt_obj)
```

```{r}
if(!exists("merged_subset_obj")){
  merged_subset_obj <- readRDS("results/day0_day50/seurat/iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_B124_seurat_obj_v5.RDS")
}

# Add metadata and idents
merged_subset_obj$id <- factor(merged_subset_obj$orig.ident, 
                               levels=unique(merged_subset_obj$orig.ident))
merged_subset_obj$seurat_clusters <- merged_subset_obj$SCT_snn_res.1
Idents(merged_subset_obj) <- merged_subset_obj$orig.ident

merged_subset_obj #60184 features across 62,544 samples within 2 assays 
table(merged_subset_obj$batch)
table(merged_subset_obj$time_point)
```

```{r fig.width=10, fig.height=10}
# pdf("figures/day0_day50/filtered_merged_results/batches_124/iPSC_MN_day0-day50_filtered_merged_soupX_normalized_B124_UMAP.pdf", height = 15, width = 15)
custom_dimplot(metadata = merged_subset_obj@meta.data,
               dimred_colnames = "umap",
               label_samples = FALSE,
               custom_colors = sample_id_colors,
               colorby = "id")
...
**Objective metric** 
  - use the amount of "density" of day0 cells in their cluster, since day0 should be 100% homogeneous and fewer clusters/cells found in later time-points

bad integration - too stringent and "forces" the cells from different batches into the same cluster. Example, we don't want to force the superimposition of day0 onto day50

*Possible Layer Groups*
- 1 layer per sample (19 SCT models)
  - integrates well, but day0 is quite dispersed across other time points (k = 10, k=5)
  - integrates well, but day0 is quite dispersed across other time points (k = 10) and stringent filtering really removes too many cells and causes error with k=5
  - using "reference" UMAPs of days0 to day30 does improve the clustering of day0 cells. 
  - using preserve order vs default does change the embedding significantly 
    - try preserve order with day50 --> day0 (latest to earliest) might improve this. 
    **perhaps preserve order + k=5 + med/low filter stringency + reference umaps best**
  
- 1 layer per time-point (15 SCT models)
  - little to no improvement in batch with the later dates with stringent or non-stringent filtering

- 1 layer per batch (4 SCT models)
  - didn't try as I expect it will not be better than per-time point 

- 1 layer per "dataset" (2 SCT models) where dataset1 is the original 15 samples, and dataset2 is the following 4 samples. 
    - didn't try as I expect it will not be better than per-time point
    - **ideally**, I would be able to keep dataset1 exactly as-is and then project dataset2 per individual sample onto dataset1 
    - probably could do that extracting the UWOT UMAP model from dataset1 and then use UWOT directly with the additional new samples 

https://github.com/satijalab/seurat/issues/7217
https://github.com/satijalab/seurat/issues/7318
...
#ensure meta cols are available
merged_norm_obj$id <- factor(merged_norm_obj$orig.ident, levels=unique(merged_norm_obj$orig.ident))
merged_norm_obj$dataset <- ifelse(merged_norm_obj$batch == "batch4", "dataset2", "dataset1")
merged_norm_obj$time_point_factor <- as.factor(merged_norm_obj$time_point)

table(merged_norm_obj$id)

# dims of reduction to use for UMAP
dims <- 1:30

# re-order the layers from latest to earliest
new_order <- rev(names(merged_norm_obj[["SCT"]]@SCTModel.list))
# merged_norm_obj[["SCT"]]@SCTModel.list <- merged_norm_obj[["SCT"]]@SCTModel.list[new_order]

# integrate the layers
merged_rpca_obj <- merged_norm_obj %>% 
  IntegrateLayers(
    ...
    reference = c(1:11),
    preserve.order = TRUE, #keep time-course samples in order given in sct list
    verbose = TRUE)
...
merged_rpca_obj <-  IntegrateLayers(
      ...
      orig.reduction = "pca",
      new.reduction = "rpca",
      reference = c(1:11), # select iPSC day0 to day35 as reference datasets
      normalization.method = 'SCT', #why is this required?
      assay = "SCT",
      k.anchor = 5,
      k.weight = 100, #default
...
```{r eval=FALSE}
merged_rpca_obj <- readRDS("results/day0_day50/seurat/iPSC_MN_time-course_filtered_merged_soupX_normalized_RPCAIntegration_seurat_obj_v5.RDS")

merged_rpca_obj
```

```{r eval=FALSE}
# NOTE: splitting removes the individual 15 models, BUT does not create any SCT models based on the splitting factor. So it cannot be used in integrateLayers. 
dataset1 <- subset(merged_norm_obj, subset = dataset == "dataset1")
dataset2 <- subset(merged_norm_obj, subset = dataset == "dataset2")
DefaultAssay(dataset2) <- "RNA"
dataset2[["RNA"]] <- JoinLayers(dataset2[["RNA"]])

ds1_SCT <- CreateSCTAssayObject(
  counts = dataset1[["SCT"]]@counts,
  
  scale.data = FetchResiduals(dataset1,
                              features = VariableFeatures(dataset1),
                              assay = "SCT"),
  umi.assay = "RNA",
  min.cells = 0,
  min.features = 0,
  SCTModel.list = NULL)
ds1_SCT@var.features <- VariableFeatures(dataset1[["SCT"]])

ds1_SCT
DefaultAssay(dataset1) <- "RNA"
dataset1[["RNA"]] <- JoinLayers(dataset1[["RNA"]])
dataset1[["SCT"]] <- NULL
dataset1[["SCT"]] <- ds1_SCT

DefaultAssay(dataset1) <- "SCT"
DefaultAssay(dataset2) <- "SCT"


merged_rpca_obj <- merge(dataset1, dataset2)
merged_rpca_obj$split_by <- case_when(
  merged_rpca_obj$dataset == "dataset1" ~ "dataset1",
  TRUE ~ merged_rpca_obj$orig.ident
)
merged_rpca_obj[["SCT"]] <- split(merged_rpca_obj[["SCT"]], f = merged_rpca_obj$split_by)
merged_rpca_obj

##########


DefaultAssay(merged_norm_obj) <- "RNA"
#grep("^MT-", rownames(merged_norm_obj), value=TRUE) # already removed the MT genes
merged_rpca_obj <- DietSeurat(merged_norm_obj, assays = "RNA")
merged_rpca_obj[["RNA"]] <- JoinLayers(merged_rpca_obj[["RNA"]])
# re-split
merged_rpca_obj[["RNA"]] <- split(merged_rpca_obj[["RNA"]], merged_norm_obj$time_point_factor)
# merged_norm_obj[["RNA"]] <- split(merged_norm_obj[["RNA"]], f = merged_norm_obj$dataset)
merged_rpca_obj[["RNA"]]

########

# Any resplitting of the SCT assays requires re-normalization and thus changes the SCT models --> residuals --> changes the integration 
# the SCT models per indiv sample 1-15 did have the best embedding results so I don't want to replace the SCT results from those samples. 
...
Idents(merged_rpca_obj) <- merged_rpca_obj$id

r1 <- DimPlot(merged_rpca_obj, 
        reduction = "umap.rpca") +
  scale_color_manual(values = sample_id_colors) +
  labs(title = "RPCA")

r2 <- DimPlot(merged_rpca_obj,
        group.by = "batch",
        reduction = "umap.rpca") +
  scale_color_manual(values = sample_id_colors) +
  labs(title = "RPCA")

r3 <- DimPlot(merged_rpca_obj, 
        reduction = "umap.rpcaorder") +
  scale_color_manual(values = sample_id_colors) +
  labs(title = "RPCA preserve order")

r4 <- DimPlot(merged_rpca_obj,
        group.by = "batch",
        reduction = "umap.rpcaorder") +
  scale_color_manual(values = sample_id_colors) +
  labs(title = "RPCA")
...
purrr::map(c("umaprpca","umaprpcaorder"), function(x){
  ...
  rpca_3d <- make_3D_umap(metadata = merged_rpca_obj@meta.data, 
             idents_column = c(paste0(x, "_",1:3)), 
             label_cols = c("id","batch"), 
             selected_colors = sample_id_colors)
  rpca_3d
...
## RPCA logNormalized Integration 

```{r eval=FALSE}
ipsc_obj <- readRDS("results/day0_day50/seurat/iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_seurat_obj_v5.RDS")

# exmine if this object should be integrated prior to celltype annotations
rna_before_int <- ipsc_obj[["RNA"]]

ipsc_obj <-  IntegrateLayers(
      object = ipsc_obj,
      method = RPCAIntegration,
      orig.reduction = "pca",
      new.reduction = "rpca_lognorm",
      reference = c(1:11), # select iPSC day0 to day35 as reference datasets
      normalization.method = 'LogNormalize', #why is this required?
      assay = "RNA",
      k.anchor = 5,
      k.weight = 100, #default
      preserve.order = FALSE, # merge by largest sized dataset
...
# [...]
# Integrating dataset 18 with reference dataset
# Finding integration vectors
# Finding integration vector weights

rna_after_int <- ipsc_obj[["RNA"]]
rna_after_int_join <- JoinLayers(ipsc_obj[["RNA"]])
```

```{r eval=FALSE}
# rna_before_int@layers$counts.iPSC_ipsc
# rna_before_int@layers$data.iPSC_ipsc
# rna_before_int@cells@.Data[,"counts.iPSC_ipsc"]
table(rna_before_int@cells@.Data[,"counts.iPSC_ipsc"]) #86954

dim(rna_before_int@layers$scale.data) #3000 86954
dim(rna_after_int@layers$scale.data) #3000 86954

# data and counts before and after integration within layers is identical values 
identical(rna_after_int@layers$counts.iPSC_ipsc, rna_before_int@layers$counts.iPSC_ipsc) # TRUE
identical(rna_after_int@layers$data.iPSC_ipsc, rna_before_int@layers$data.iPSC_ipsc) # TRUE

# data before and after integration + join is. identical values
identical(rna_after_int_join@layers$data[,rna_before_int@cells@.Data[,"counts.iPSC_ipsc"]],
          rna_before_int@layers$data.iPSC_ipsc)  # TRUE
# counts before and after integration + join is. identical values
identical(rna_after_int_join@layers$counts[,rna_before_int@cells@.Data[,"counts.iPSC_ipsc"]], 
          rna_before_int@layers$counts.iPSC_ipsc) # TRUE

# So even scale data is NOT batch corrected in any way?? WTF
# scale.data integration is. identical values
identical(rna_after_int@layers$scale.data, rna_before_int@layers$scale.data) # TRUE?? 
# scale.data integration + join is. identical values
identical(rna_after_int_join@layers$scale.data , rna_before_int@layers$scale.data) # TRUE??
```
...
*Warning:* HarmonyMatrix is deprecated and will be removed in the future from the API in the futureWarning: Warning: The parameters do_pca and npcs are deprecated. They will be ignored for this function call and please remove parameters do_pca and npcs and pass to harmony cell_embeddings directly
...
The results are identical when changing `preserve.order`
* so changing the param for HARMONY Integration order does not affect anything.
* I'm guessing since its a seurat wrapper, when underlying harmony alg is invoked it will just order the datasets based on its defaults.

```{r eval=FALSE}
# for some reason, the harmony integration just over-writes the last harmony reduction, regardless of names being different. 
merged_harmony_obj2 <- IntegrateLayers(
    object = merged_norm_obj,
    method = HarmonyIntegration,
    orig.reduction = "pca",
...
# identical(merged_harmony_obj1@reductions$harmony@cell.embeddings,
#           merged_harmony_obj2@reductions$harmonyorder@cell.embeddings) # TRUE

DimPlot(merged_harmony_obj1,
        reduction = "umap.harmony") +
  scale_color_manual(values = colors_vector)

DimPlot(merged_harmony_obj2, 
        reduction = "umap.harmonyorder") +
    scale_color_manual(values = colors_vector)
```
...
```{r eval=TRUE}
purrr::map(c("harmony"), function(x){
  ...
  htmlwidgets::saveWidget(harmony_3d, glue("figures/day0_day50/filtered_merged_harmony/merged_{x}_obj_iPSC_MN_seurat_UMAP.html"))
})
```
...
```{r eval=TRUE}
  ...
  merged_norm_batchcorr <- SCTransform(temp,
                             assay="RNA",
                             new.assay.name = "SCT",
...
# bc_3d <- make_3D_umap(metadata = merged_norm_batchcorr@meta.data, 
#                  idents_column = c(paste0("umap_", 1:3)), 
#                  label_cols = c("id","batch"), 
#                  selected_colors = colors_vector)
#   
# htmlwidgets::saveWidget(bc_3d, glue("figures/day0_day50/filtered_merged_batchcorrected/merged_batchcorrected_obj_iPSC_MN_seurat_UMAP.html"))
``` 
