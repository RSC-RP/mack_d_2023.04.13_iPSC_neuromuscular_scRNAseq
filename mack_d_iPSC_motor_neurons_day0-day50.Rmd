...
dims <- 1:30
# had error when run with defaul k.anchor =  5.
# Error Number of anchor cells is less than k.weight. Consider lowering k.weight to less than 92 or increase k.anchor.
merged_rpca_obj <-  IntegrateLayers(
...
```
```{r fig.height=10, fig.width=10}
DimPlot(merged_rpca_obj, reduction = "umap.rpca") +
  scale_color_manual(values = colors_vector)

DimPlot(merged_rpca_obj, reduction = "umap.rpcaorder") +
  scale_color_manual(values = colors_vector)
```
...
# defaults to Layers which are the sample IDs in this case
merged_harmony_obj1 <- IntegrateLayers(
    object = merged_norm_obj,
...
# so changing the param for HARMONY Integration order does not affect anything. I'm guessing since its a seurat wrapper, when underlying harmony alg is invoked it will just order the datasets based on its defaults. 

# identical(merged_harmony_obj1@reductions$harmony@cell.embeddings,
#           merged_harmony_obj2@reductions$harmonyorder@cell.embeddings)
```
...
```{r eval=TRUE}
merged_harmony_obj <- merged_harmony_obj1
merged_harmony_obj@reductions[["harmonyorder"]] <- merged_harmony_obj2@reductions$harmony
merged_harmony_obj@reductions[["umap.harmonyorder"]] <- merged_harmony_obj2@reductions$umap.harmonyorder
