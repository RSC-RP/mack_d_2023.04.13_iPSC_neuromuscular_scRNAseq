...
current_obj <- "iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_seurat_obj.RDS"
...
}else if(!exists("preprocess_merge_obj") & is.null(current_obj) ){
  message(glue("Loading the merged object {preprocess_merge_obj}"))
...
pdf("figures/day0_day50/iPSC_MN_seurat_PCA_elbow_noFilter.pdf", height = 10, width = 10)
...
```{r fig.width=10}
# png("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_filtered_seurat_barplot.png",height = 8, width = 12, unit="in", res=150)
threshold_summary %>% 
...
```{r}
# pdf("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_filtered_seurat_mito_thresholds.pdf", height = 7, width = 10)
...
est_first_pass <- purrr::map(names(soup_channels), function(x){
  ...
  min <- 1
  sc <- soup_channels[[x]]
  outfile2 <- glue("figures/day0_day50/soupX/{x}_autoEstCont_figure.pdf")
  tryCatch(expr = {
...
job::job({
  saveRDS(final_estimates, "results/soupX_counts/mack_iPSC_MN_soupX_channel_objs_list.RDS")
...
```{r eval=FALSE}
marker_genes <- quickMarkers(toc = soup_channels$iPSC_day8$toc,
             clusters = soup_channels$iPSC_day8$metaData$clusters,
             N = 25,
             FDR = 0.05,
             expressCut = 0.9)

cells <- estimateNonExpressingCells(
  soup_channels$iPSC_day8,
  nonExpressedGeneList = nonExp_list,
  # clusters = NULL,
  maximumContamination = 1,
  FDR = 0.05
)

nonExp_list <- read.csv("allen_institute_azimuth_marker_gene_list.csv") %>% 
  mutate(Label = gsub("\\s{1,}|\\/|-","_", Label)) %>% 
  pull(Markers , name = Label) %>% 
  as.list() %>% 
  sapply(.,  function(x) str_split(gsub("\\.","-",x), pattern = ", ")) %>% 
  sapply(., function(x) x[x %in% rownames(merged_obj)])


# https://github.com/constantAmateur/SoupX/issues/102
# plotMarkerDistribution(sc = soup_channels$iPSC_day8, maxCells = 150, tfidfMin = 0.7)
plotMarkerDistribution(sc = soup_channels$iPSC_day8, 
                       nonExpressedGeneList = nonExp_list,
                       maxCells = 150, 
                       tfidfMin = 1)
...
# png("figures/day0_day50/soupX/iPSC_MN_contamination_estimates.png", height = 5, width = 8, units = "in", res = 150)
est_plot
# dev.off()
```
...
```{r eval=FALSE}
job::job({
  sapply(names(soupx_objs), function(x) saveRDS(soupx_objs[[x]],glue("results/individual_objs/mack_{x}_motor_neuron_filtered_soupX_seurat_obj.RDS")))
}, import = c("soupx_objs"))
```
...
```{r}
# table(merged_filt_obj@meta.data$time_point_day)
```
...
```{r eval=FALSE}
job::job({
  saveRDS(merged_norm_obj, 
          "results/day0_day50/seurat/iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_seurat_obj.RDS")
...
length(unique(merged_norm_obj$seurat_clusters))
...
DimPlot(merged_norm_obj, dims = c(1,2)) +
  scale_color_manual(values = colors)
# dev.off()
```
...
fig <- fig %>% 
  add_markers( 
    marker = list(size = 1), 
    text = merged_norm_obj@meta.data$orig.ident,
      hoverinfo = 'text')
...
```{r}
median_umis_filt <- merged_norm_obj@meta.data %>% 
  mutate(id=factor(orig.ident, levels=unique(orig.ident))) %>% 
  select(id, median_umi_counts_per_cell, batch) %>% 
  distinct() %>% 
...
job::job({
  ...
  saveRDS(merged_norm_batchcorr, 
          "results/day0_day50/seurat/iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_batchcorrected_seurat_obj.RDS")
}, import = c("merged_norm_batchcorr"))
```

```{r}
merged_norm_batchcorr <- readRDS(file.path(current_path, "iPSC_motor_neuron_day0-day50_time-course_filtered_merged_soupX_normalized_batchcorrected_seurat_obj.RDS"))
...
```{r fig.height=10}
pdf("figures/day0_day50/filtered_merged_batchcorrected/sct_transform_100PCs_elbow_plot.pdf", height = 7, width=10)
ElbowPlot(merged_norm_batchcorr,ndims = 50, reduction = "pca")
dev.off()

pdf("figures/day0_day50/filtered_merged_batchcorrected/transform_1-4PCs_loadings.pdf", height = 10, width=7)
VizDimLoadings(merged_norm_batchcorr, dims = 1:4, reduction = "pca")
dev.off()

pdf("figures/day0_day50/filtered_merged_batchcorrected/byLouvainCluster_sct_UMAP.pdf",height = 10, width= 17)
...
```{r fig.height=10}
filt_d1 <- DimPlot(merged_norm_batchcorr, 
        group.by = c("batch","id"),
        label = T, 
        repel = T,
        label.size = 7) + 
...
filt_s1 <- DimPlot(merged_norm_batchcorr, 
        group.by = c("id"),
        split.by = "batch",
        label = T, 
        repel = T,
...
```{r fig.height=10}
fig <- plot_ly(merged_norm_batchcorr@meta.data,
               x = ~umap_1, y = ~umap_2, z = ~umap_3, 
               color = ~id,
               colors = colors)
fig <- fig %>% 
...
purrr::map(names(marker_genes), function(type){
  outfile <- glue("figures/day0_day50/filtered_merged_results/day0_day50/iPSC_MN_expression_{type}_marker_genes.png")
  
  png(outfile, height = 10, width = 20, units = "in", res = 100)
  print(FeaturePlot(merged_norm_batchcorr, 
...
pdf("figures/day0_day50/filtered_merged_results/day0_day50/iPSC_MN_seurat_topVarFeatures.pdf", height = 5, width = 10)
plot2
dev.off()
```

```{r}
median_umis_filt <- merged_norm_batchcorr@meta.data %>% 
  mutate(id=factor(orig.ident, levels=unique(orig.ident))) %>% 
  select(id, median_umi_counts_per_cell, batch) %>% 
  distinct() %>% 
