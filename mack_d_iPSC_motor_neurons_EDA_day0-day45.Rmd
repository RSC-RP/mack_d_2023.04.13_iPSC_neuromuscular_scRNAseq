...
raw_mats <- dir("resources", pattern = "raw_feature_bc", recursive = TRUE, full.names = TRUE) %>% 
  grep("D50", ., value=TRUE, invert = TRUE)
...
```{r}
current_path <- file.path("results/day0_day45/seurat")
current_obj <- "iPSC_motor_neuron_time-course_filtered_merged_soupX_normalized_seurat_obj.RDS"
...
if(!exists("filt_obj") & !is.null(current_obj)){
  ...
  merged_obj <- readRDS(file.path(current_path,"intermediate_objs",current_merge_obj))
}
```
...
# png("figures/preprocessed/iPSC_MN_seurat_number_cells_per_day_noFilter.png", 
...
# png("figures/preprocessed/cellranger_qc_metrics_byBatch_median_UMIs.png", height = 8, width = 7, units = "in",res = 150)
median_umis
# dev.off()
```
...
```{r}
doublet_scores <- read.csv("results/doublet_scores/mack_iPSC_MN_scDblFinder_scores.csv") %>% 
  mutate(id=factor(orig.ident, levels=unique(orig.ident)),
         barcode=X) %>%
  left_join(., sample_manifest, by=c("orig.ident"="sample_id")) %>%
...
job::job({
  ...
  filter_objs <- purrr::map(names(seurat_objs_dbls), function(x){
        ...
        if(!file.exists(outfile) | !file.exists(out_summary)){
          ...
          print(glue("Saving {outfile}"))
          base::saveRDS(merged_filt_obj, outfile)
          ...
          merged_filt_obj <- readRDS(outfile)
        }
        return(merged_filt_obj)
      })
...
```{r}
threshold_summary <- purrr::map_dfr(dir("results/day0_day50/seurat/filtering_stats/",
                                        pattern = "filtering_threshold_summary.csv", 
                                        full.names = TRUE), function(x){
  read.csv(x)
}) %>% 
  mutate(id=paste0("iPSC_",sample)) %>% 
  mutate(percent=round(output_number_cells/input_number_cells * 100, digits = 1)) %>% 
  left_join(., sample_manifest, by=c("id"="sample_id")) %>% 
  mutate(id=factor(id, levels=levels(doublet_scores$id))) %>% 
  arrange(id)


threshold_summary %>% head()
dim(threshold_summary) #14 36
```

```{r fig.width=10}
# png("figures/filtering/mack_ipsc_motor_neuron_filtered_seurat_barplot.png",height = 8, width = 12, unit="in", res=150)
threshold_summary %>% 
...
```{r}
# pdf("figures/filtering/mack_ipsc_motor_neuron_filtered_seurat_mito_thresholds.pdf", height = 7, width = 10)
...
```{r}
seurat_objs_filt_d5_d8 <- dir("results/individual_objs/first_pass_filter", 
                          pattern = "mack_.+_motor_neuron_filtered_seurat_obj.RDS",
                          full.names = TRUE) %>% 
    set_names(str_split_fixed(basename(.), "_", n=4)[,2])

filter_objs_d5_d8 <- purrr::map(names(seurat_objs_filt_d5_d8), function(x) {
    outfile <- glue::glue("results/individual_objs/mack_{x}_motor_neuron_filtered_seurat_obj.RDS")
    obj <- readRDS(seurat_objs_filt_d5_d8[[x]]) 
    sub_filt_obj <- subset(
            x = obj,
...
names(filter_objs_d5_d8) <- paste0("iPSC_", names(seurat_objs_filt_d5_d8))
...
# write.csv(threshold_summary_update, "results/day0_day50/seurat/filtering_stats/mack_iPSC_MN_day0-day50_cell_filtering_threshold_summary.csv", row.names = FALSE)
```
...
```{r fig.width=10}
# png("figures/day0_day50/filtering/mack_iPSC_motor_neuron_day0-day50_filtered_seurat_barplot_update.png",height = 8, width = 12, unit="in", res=150)
threshold_summary_update %>% 
  pivot_longer(cols = c(input_number_cells,output_number_cells), 
               names_to = "status",
               values_to = "number_of_cells") %>% 
...
if(!exists("filter_objs")){
  seurat_objs_filt <- dir("results/individual_objs", 
                          ...
                          full.names = TRUE) %>% 
    set_names(str_split_fixed(basename(.), "_", n=4)[,2])
  seurat_objs_filt <- seurat_objs_filt[-grep("day50", names(seurat_objs_filt))]
...
length(filter_objs) #14
# head(filter_objs)

if(!exists("soup_channels")){
  soup_channels <- purrr::map(names(filter_objs), function(x){
  ...
  names(soup_channels) <- names(filter_objs)
  # length(soup_channels) #14
}

# job::job({
#   saveRDS(soup_channels, "results/soupX_counts/mack_iPSC_MN_soupX_channel_objs_list.RDS")
# }, import = c("soup_channels"))

est_first_pass <- purrr::map(names(soup_channels), function(x){
  print(x)
  min <- 1
  sc <- soup_channels[[x]]
  outfile2 <- glue("figures/day0_day45/soupX/{x}_autoEstCont_figure.pdf")
  tryCatch(expr = {
    ...
    print(glue("{x} Too homogenous for reliable estimate"))
    return(NULL)
  })
...
names(est_first_pass) <- names(soup_channels)
...
save_corrected_counts <- purrr::map(names(final_estimates), function(x){
  outdir <- file.path("results/soupX_counts", x)
...
# job::job({
#   saveRDS(final_estimates, "results/soupX_counts/mack_iPSC_MN_soupX_channel_adj_objs_list.RDS")
# }, import = c("final_estimates"))
```
...
# write.csv(estimates_df, "results/seurat/soupX_counts/mack_iPSC_MN_soupX_contamination_estimates.csv", row.names = FALSE)
```

```{r}
estimates_df <- read.csv("results/seurat/soupX_counts/mack_iPSC_MN_soupX_contamination_estimates.csv") %>% 
  mutate(id=factor(sample, levels=levels(doublet_scores$id)))
...
# png("figures/soupX/iPSC_MN_contamination_estimates.png", height = 5, width = 8, units = "in", res = 150)
est_plot
# dev.off()
```
...
```{r eval=FALSE}
outs <- dir("results/seurat/soupX_counts",pattern = "^iPSC", full.names = TRUE)
...
```{r eval=FALSE}
job::job({
  sapply(names(soupx_objs), function(x) saveRDS(soupx_objs[[x]],glue("results/seurat/individual_objs/mack_{x}_motor_neuron_filtered_soupX_seurat_obj.RDS")))
}, import = c("soupx_objs"))
```
...
#  2 layers present: counts, data
merged_filt_obj 
# saveRDS(merged_filt_obj,"results/seurat/iPSC_motor_neuron_time-course_filtered_merged_soupX_seurat_obj.RDS")
```
...
```{r}
# https://github.com/regan-hamel/h-iPSCs-MNI/blob/master/R%20workflow_iMNs_LThiry_2020.Rmd
marker_genes <- list(MNPCs = c("CHAT", "ISL1", "ISL2", "MNX1", "OLIG2", "NEUROG2"),
...
job::job({
  ...
  merged_filt_obj <- SCTransform(merged_filt_obj,
                           assay="RNA",
                           new.assay.name = "SCT",
  ...
  merged_filt_obj <- merged_filt_obj %>% 
  ...
  merged_filt_obj <- AddMetaData(merged_filt_obj,
                                 as.data.frame(merged_filt_obj@reductions$umap@cell.embeddings))
  
  merged_filt_obj
}, import = c("merged_filt_obj"))

```

```{r eval=FALSE}
job::job({
  #/active/taylor_s/people/jsmi26/CP-iPSC_motor_neurons/mack_d_2023.04.13_iPSC_neuromuscular_scRNAseq/
  saveRDS(merged_filt_obj, 
          "results/seurat/iPSC_motor_neuron_time-course_filtered_merged_soupX_normalized_seurat_obj.RDS")
}, import = c("merged_filt_obj"))
```

```{r}
merged_filt_obj$id <- factor(merged_filt_obj$orig.ident, levels = c(unique(merged_filt_obj$orig.ident)))

# merged_filt_obj$SCT_snn_res.1.5 %>% unique %>% length()
# merged_filt_obj$SCT_snn_res.1 %>% unique %>% length()

merged_filt_obj$seurat_clusters <- merged_filt_obj$SCT_snn_res.1
# merged_filt_obj$seurat_clusters %>%  unique() %>%  length
 
Idents(merged_filt_obj) <- merged_filt_obj$SCT_snn_res.1
```

```{r fig.height=10}
# pdf("figures/filtered_merged_results/sct_transform_100PCs_elbow_plot.pdf", height = 7, width=10)
# ElbowPlot(merged_filt_obj,ndims = 50, reduction = "pca")
# dev.off()
# # 
# pdf("figures/filtered_merged_results/transform_1-4PCs_loadings.pdf", height = 10, width=7)
# VizDimLoadings(merged_filt_obj, dims = 1:4, reduction = "pca")
# dev.off()

pdf("figures/filtered_merged_results/byLouvainCluster_sct_UMAP.pdf",height = 10, width= 17)
DimPlot(merged_filt_obj, dims = c(1,2)) +
  scale_color_manual(values = colors)
dev.off()
```

```{r fig.height=10}
filt_d1 <- DimPlot(merged_filt_obj, 
        group.by = c("batch","id"),
        label = T, 
        repel = T,
        label.size = 7) + 
...
png("figures/filtered_merged_results_iPSC_MN_seurat_UMAP_batch_day_filtered.png", height = 15, width = 25,
    units = "in", res = 150)
filt_d1
dev.off()

filt_s1 <- DimPlot(merged_filt_obj, 
        group.by = c("id"),
        split.by = "batch",
        label = T, 
        repel = T,
...
png("figures/filtered_merged_results_iPSC_MN_seurat_UMAP_splitBy_batch_day_filtered.png",
    height = 15, width = 25, units = "in", res = 150)
filt_s1
dev.off()
```
...
```{r fig.height=10}
fig <- plot_ly(merged_filt_obj@meta.data,
...
fig <- fig %>% 
  add_markers( 
    marker = list(size = 1), 
    text = merged_filt_obj@meta.data$orig.ident,
      hoverinfo = 'text')
...
```{r}
Idents(merged_filt_obj) <- merged_filt_obj$orig.ident
purrr::map(names(marker_genes), function(type){
  outfile <- glue("figures/filtered_merged_results/iPSC_MN_expression_{type}_marker_genes.png")
  
  png(outfile, height = 10, width = 20, units = "in", res = 100)
  print(FeaturePlot(merged_filt_obj, 
                    features = marker_genes[[type]],
                    label = TRUE))
  dev.off()
...
```{r}
# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(merged_filt_obj), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(merged_filt_obj)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)

pdf("figures/filtered_merged_results/iPSC_MN_seurat_topVarFeatures.pdf", height = 5, width = 10)
plot2
dev.off()
```
...
```{r}
median_umis_filt <- merged_filt_obj@meta.data %>% 
  mutate(id=factor(orig.ident, levels=unique(orig.ident))) %>% 
  select(id, median_umi_counts_per_cell, batch) %>% 
  distinct() %>% 
